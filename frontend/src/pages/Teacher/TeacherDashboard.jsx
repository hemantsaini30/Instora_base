...
const TeacherDashboard = () => {
  ...
  useEffect(() => {
    ...
    setStudents([]); setAttendance({}); setMessage(''); setIsLocked(false)
    getAllStudents().then(res => {
      ...
      getAttendanceByBatchAndDate(selectedBatch, selectedDate).then(attRes => {
        ...
        if (Object.keys(existing).length > 0) { setAttendance(existing); setIsLocked(true) }
        else { setAttendance(initial); setIsLocked(false) }
      }).catch(() => { setAttendance(initial) })
    })
    getBatchAttendanceSummary(selectedBatch).then(res => setSummary(res.data.data)).catch(() => {})
    getDatewiseAttendance(selectedBatch).then(res => setDatewiseData(res.data.data)).catch(() => {})
  ...
  const getPercentageColor = (pct) => pct >= 75 ? 'text-emerald-600' : pct >= 50 ? 'text-amber-600' : 'text-red-500'
  ...
  return (
    <TeacherLayout>
      <div className="p-4 md:p-8">
        ...
        <div className="flex gap-4 mb-6 flex-wrap">
          <div className="flex flex-col gap-1">
            ...
            <select value={selectedBatch} onChange={e => setSelectedBatch(e.target.value)}
              className="px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-purple-500 min-w-48">
        ...
        {!selectedBatch && <div className="text-center py-16 text-gray-400"><p className="text-lg">Select a batch to get started</p></div>}
        {selectedBatch && tab === 'mark' && (
          <div>
            {message && (
              <div className={`text-sm px-4 py-3 rounded-lg mb-4 flex items-center justify-between ${message.includes('success') ? 'bg-emerald-50 border border-emerald-200 text-emerald-600' : 'bg-red-50 border border-red-200 text-red-600'}`}>
                <span>{message}</span>
                {message.includes('success') && <button onClick={handleMarkAgain} className="text-xs bg-emerald-600 text-white px-3 py-1 rounded-lg ml-4">Mark again</button>}
              </div>
            )}
            {isLocked && !message && (
            ...
            {students.length === 0 ? <div className="text-center py-12 text-gray-400"><p>No students in this batch</p></div> : (
              <div className={`bg-white border rounded-xl overflow-hidden ${isLocked ? 'border-blue-200 opacity-75 pointer-events-none' : 'border-gray-200'}`}>
                ...
                {students.map((student, i) => (
                  <div key={student._id} className={`flex items-center justify-between px-5 py-4 ${i !== students.length - 1 ? 'border-b border-gray-100' : ''}`}>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => setAttendance({ ...attendance, [student._id]: 'present' })}
                        className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-colors ${attendance[student._id] === 'present' ? 'bg-emerald-500 text-white' : 'bg-gray-100 text-gray-500'}`}>Present</button>
                      <button onClick={() => setAttendance({ ...attendance, [student._id]: 'absent' })}
                        className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-colors ${attendance[student._id] === 'absent' ? 'bg-red-400 text-white' : 'bg-gray-100 text-gray-500'}`}>Absent</button>
                    </div>
                  </div>
                ))}
                {!isLocked && (
                  <div className="px-5 py-4 border-t border-gray-100 bg-gray-50 flex justify-end">
                    <button onClick={handleSave} disabled={saving}
                      className="bg-purple-700 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-purple-800 disabled:opacity-50">
        ...
        {selectedBatch && tab === 'summary' && (
          <div className="bg-white border border-gray-200 rounded-xl overflow-x-auto">
            {summary.length === 0 ? <div className="text-center py-12 text-gray-400"><p>No attendance data yet</p></div> : (
